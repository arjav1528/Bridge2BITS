...
export const MarqueeSection = () => (
    // Remove all container constraints and make it truly full width
    <div className="w-screen relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] overflow-hidden">
        <Marquee
            ...
            pauseOnHover={true}
            direction="right"
            className="py-8 bg-transparent flex items-center space-x-6"
        >
            {testimonials.map(({ id, pfpUrl, name, text, college }) => (
                <div key={id} className="flex flex-col mx-4 p-6 bg-gray-900 rounded-2xl shadow-lg border-2 border-yellow-400/50 w-80 h-64 hover:shadow-xl hover:shadow-yellow-400/50 transition-all duration-300 hover:scale-105 flex-shrink-0">
